...
function printHelp(): void {
  // Tight-spaced, symmetrical "WAY OF" logo (exactly 54 characters wide)
  const logo = [
    "██╗    ██╗   █████╗   ██╗   ██╗     ██████╗   ███████╗",
    "██║    ██║  ██╔══██╗  ╚██╗ ██╔╝    ██╔═══██╗  ██╔════╝",
    "██║ █╗ ██║  ███████║   ╚████╔╝     ██║   ██║  █████╗  ",
...
interface InstallOptions {
  ...
  settingsAccepted?: boolean; // Track the user's answer to settings prompt
}
    ...
    if (comp.name === "settings" && !opts.yes && opts.skills.length === 0) {
      if (!opts.settingsPrompted) {
        ...
        if (opts.dryRun) {
          ...
          opts.settingsPrompted = true;
          opts.settingsAccepted = ok;
          if (!ok) {
      ...
      } else if (!opts.settingsAccepted) {
        // Already prompted and user said no - skip silently
        skipped += comp.files.length;
        continue;
      }
    }
