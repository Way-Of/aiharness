...
async function executeFallbackSearch(query, maxTokens, signal) {
    const text = await callExaMcp(WEB_SEARCH_TOOL, {
...
export async function executeCodeSearch(_toolCallId, params, signal) {
    ...
    try {
        ...
        if (codeContextToolMissing) {
            ...
            try {
                text = await callExaMcp(CODE_CONTEXT_TOOL, {
                    query,
                    tokensNum: maxTokens,
                }, signal);
                mode = "code-context";
                ...
                if (!isMissingMcpToolError(message))
                    throw err;
